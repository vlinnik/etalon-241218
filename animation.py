...
class Animation(QLabel):
    ...
    def _preload(self):
        if not self._movie or PlaybackHint.Reversed not in self.getPlaybackHints():
            return
    ...
    def setPlaybackHints(self,hint: PlaybackHint):
        ...
        if PlaybackHint.Reversed in self.playbackHints and self._movie and self._movie.frameCount()==0:
            self._preload( )
    ...
    def setRunning(self,running: bool):
        ...
        if running and PlaybackHint.Reversed not in self.playbackHints:
            self._movie.start()
        else:
            self._movie.stop( )
            if PlaybackHint.Reversed in self.playbackHints and running and self._movie.frameCount()>0:
    ...
    def _frameChanged(self,frame: int):
        if PlaybackHint.Bounce in PlaybackHint(self._hint) and frame>0 and not self.isRunning():
            QTimer.singleShot(self._movie.nextFrameDelay(), lambda: self._movie.jumpToFrame( frame-1 ))
        if PlaybackHint.Ceaseless in PlaybackHint(self._hint) and not self.isRunning() and frame!=0:
            if frame<self._movie.frameCount( ):
                QTimer.singleShot(self._movie.nextFrameDelay(), lambda: self._movie.jumpToFrame( frame+1 ))
            else:
                QTimer.singleShot(self._movie.nextFrameDelay(), lambda: self._movie.jumpToFrame( 0 ))
        if PlaybackHint.Reversed in PlaybackHint(self._hint) and self.isRunning():
            if frame>0:
                QTimer.singleShot(self._movie.nextFrameDelay(), lambda: self._movie.jumpToFrame( frame-1 ))
